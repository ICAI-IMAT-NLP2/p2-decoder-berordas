...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        attn_out = self.self_attention(y, mask=mask)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        allowed = torch.tril(torch.ones(seq_len, seq_len, device=device, dtype=torch.bool))
        mask = (~allowed).unsqueeze(0).expand(bsz, -1, -1)
...
class TransformerForLanguageModeling(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        hidden = self.transformer_decoder(input_ids)
